    ...
    <div className="min-h-screen bg-white text-black flex flex-col md:flex-row font-sans relative">
      ...
      <div className="fixed top-8 left-8 z-[120]">
        <motion.button 
          whileTap={{ scale: 0.95 }}
          onClick={() => router.push("/")} 
          className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-black/40 hover:text-black transition-colors group cursor-pointer"
        >
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center px-8 md:px-20 py-24 border-r border-black/5 relative overflow-hidden">
        <div className="max-w-sm w-full mx-auto relative z-10">
          <div className="mb-12">
            ...
            <h1 className="text-4xl font-bold uppercase tracking-tighter leading-none mb-3 text-left">
          ...
          {step === 1 && (
            <div className="space-y-4 mb-8">
              <button onClick={handleGoogleAuth} disabled={loading} className="w-full py-4 bg-[#F9F9F9] text-black border border-black/5 rounded-sm flex items-center justify-center gap-3 hover:bg-black hover:text-white transition-all duration-300 cursor-pointer">
                <Globe size={18} />
            ...
            {step === 1 ? (
              <motion.div key="step1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                <div className="flex gap-1 bg-[#F9F9F9] p-1 rounded-sm mb-8">
                  <button onClick={() => setAuthMode("register")} className={`flex-1 py-2 text-[9px] font-bold uppercase tracking-widest transition-all cursor-pointer ${authMode === "register" ? "bg-white shadow-sm" : "text-black/30"}`}>Register</button>
                  <button onClick={() => setAuthMode("login")} className={`flex-1 py-2 text-[9px] font-bold uppercase tracking-widest transition-all cursor-pointer ${authMode === "login" ? "bg-white shadow-sm" : "text-black/30"}`}>Existing Member</button>
                </div>
                ...
                <form onSubmit={authMode === "register" ? handleSendOTP : handlePasswordLogin} className="space-y-6">
                  ...
                  {authMode === "register" && (
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold uppercase tracking-widest text-black/40 px-1 block text-left">Mobile Number</label>
              ...
              <motion.form key="step2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} onSubmit={handleVerifyOTP} className="space-y-6 text-center">
                ...
                <button type="button" onClick={() => setStep(1)} className="text-[10px] uppercase tracking-widest font-bold text-black/30 hover:text-black cursor-pointer bg-transparent border-none">Edit Details</button>
              </motion.form>
            ) : (
          ...
          <footer className="mt-16 pt-8 border-t border-black/5 flex flex-col gap-4 text-[9px] uppercase tracking-widest font-mono text-black/30 text-left">
...
export default function HybridLoginClient() {
  return (
...
const AuthButton = ({ loading, text, icon }: { loading: boolean, text: string, icon: React.ReactNode }) => (
  <motion.button 
    ...
    type="submit" 
    disabled={loading} 
    className="w-full py-4 bg-black text-white rounded-sm flex items-center justify-center gap-3 hover:bg-[#F3D7A7] hover:text-black transition-all duration-500 group cursor-pointer"
  >
